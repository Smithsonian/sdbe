...
static void * _threaded_receiver(void *arg) {
	...
	if (obtain_data_lock(dest) == 0) {
		dest->frame_size = 2056; // REDO: We need some other way to set the frame size
		...
		if (frames_received == 0) {
			...
			do {
				...
				if (num_bytes_or_err < 0) {
					if (num_bytes_or_err == ERR_NET_READ_TIMEOUT) {
...
static void * _threaded_writer(void *arg) {
	...
	if (obtain_data_lock(src) == 0) {
		src->frame_size = 2056; // REDO: We need some other way to set the frame sizes
