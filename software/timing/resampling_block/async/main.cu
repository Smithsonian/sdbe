...
typedef struct aphids_resampler {
    ...
    cufftHandle cufft_plan[NUM_STREAMS][3];
    cudaStream_t stream[NUM_STREAMS];
    cudaEvent_t tic[NUM_STREAMS],toc[NUM_STREAMS];
...
int aphids_resampler_init(aphids_resampler_t *resampler, int _deviceId) {
  ...
  for (int i=0; i< NUM_STREAMS; i++){
	HANDLE_ERROR( cudaStreamCreate(&(resampler->stream[i])) );
	cudaEventCreate(&(resampler->tic[i]));
	cudaEventCreate(&(resampler->toc[i]));
  }
 ...
 */
  ...
  for (int i=0; i<NUM_STREAMS; i++){
  	cufftPlanMany(&(resampler->cufft_plan[i][0]), 1, &(resampler->fft_size[0]),
		inembed,1,inembed[0],
		onembed,1,onembed[0],
		CUFFT_C2R,resampler->batch[0]);
	cufftSetStream(resampler->cufft_plan[i][0],resampler->stream[i]);
  }
  reportDeviceMemInfo();
 ...
 */
  ...
  for (int i=0; i<NUM_STREAMS; i++){
  	cufftPlanMany(&(resampler->cufft_plan[i][1]), 1, &(resampler->fft_size[1]),
		inembed,1,inembed[0],
		onembed,1,onembed[0],
		CUFFT_R2C,resampler->batch[1]);
	cufftSetStream(resampler->cufft_plan[i][1],resampler->stream[i]);
  }
  reportDeviceMemInfo();
 ...
 */
  ...
  for (int i=0; i<NUM_STREAMS; i++){
  	cufftPlanMany(&(resampler->cufft_plan[i][2]), 1, &(resampler->fft_size[2]),
		inembed,1,inembed[0],
		onembed,1,onembed[0],
		CUFFT_C2R,resampler->batch[2]);
	cufftSetStream(resampler->cufft_plan[i][2],resampler->stream[i]);
  }
...
int SwarmC2R(aphids_resampler_t *resampler){
  ...
  for (int i = 0; i < resampler->repeat[0]; ++i) {
	cufftExecC2R(resampler->cufft_plan[0][0],
	...
	cufftExecC2R(resampler->cufft_plan[1][0],
...
int SwarmR2C(aphids_resampler_t *resampler){
  ...
  for (int i = 0; i < resampler->repeat[1]; ++i) {
	cufftExecR2C(resampler->cufft_plan[0][1],
	...
	cufftExecR2C(resampler->cufft_plan[1][1],
...
int Hr2dbeC2R(aphids_resampler_t *resampler){
  for (int i = 0; i < resampler->repeat[2]; ++i) {
	cufftExecC2R(resampler->cufft_plan[0][2],
	...
	cufftExecC2R(resampler->cufft_plan[1][2],
...
int aphids_resampler_destroy(aphids_resampler_t *resampler) {
  ...
  for (int i=0; i<NUM_STREAMS; ++i){
  	HANDLE_ERROR( cudaStreamDestroy(resampler->stream[i]) );
  	cudaEventDestroy(resampler->tic[i]);
  	cudaEventDestroy(resampler->toc[i]);
  	for (int j=0; j < 3; ++j){
	  cufftDestroy(resampler->cufft_plan[i][j]);
  	}
  }
...
int main(int argc, char *argv[]){
  ...
  for (int deviceId=0; deviceId < numDevices; deviceId++){
	...
	for (int i=0; i< NUM_STREAMS; i++){
	  cudaStreamSynchronize(resampler.stream[i]);
	  cudaEventRecord(resampler.tic[i]);
        }
	...
	for (int cnt=0; cnt<ROUNDS; cnt++){
	   reorderTzp_smem<<<blocks,threads,0,resampler.stream[0]>>>(resampler.gpu_A_0, resampler.gpu_B_0, BENG_BUFFER_IN_COUNTS);
	   reorderTzp_smem<<<blocks,threads,0,resampler.stream[1]>>>(resampler.gpu_A_1, resampler.gpu_B_1, BENG_BUFFER_IN_COUNTS);
	}
	...
	for (int cnt=0; cnt<ROUNDS; cnt++){
	  quantize2bit<<<blocks,threads,0,resampler.stream[0]>>>((float *) resampler.gpu_A_0, (unsigned int*) resampler.gpu_B_0, 
		(2*BENG_CHANNELS_*BENG_SNAPSHOTS*EXPANSION_FACTOR),
		QUANTIZE_THRESHOLD);
	  quantize2bit<<<blocks,threads,0,resampler.stream[1]>>>((float *) resampler.gpu_A_1, (unsigned int*) resampler.gpu_B_1, 
		(2*BENG_CHANNELS_*BENG_SNAPSHOTS*EXPANSION_FACTOR),
		QUANTIZE_THRESHOLD);
	}

	for (int i=0; i< NUM_STREAMS; i++){
	  cudaEventRecord(resampler.toc[i]);
	  cudaEventSynchronize(resampler.toc[i]);
	  cudaEventElapsedTime(&elapsedTime,resampler.tic[i],resampler.toc[i]);
	  fprintf(pFile,"%15s %5i %5i %12.4f %12.4f\n","total",deviceId, i, elapsedTime, 
