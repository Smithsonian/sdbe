...
class ListenerTX(Thread):
	...
	SGTX_CMD = "./transmit {0} {1}"
	SGTX_BLOCKS = 1
	SGTX_BENG_FRAMES_PER_BLOCK = 40
	...
	def process_start(self):
		...
		print "Popen command: ", self.SGTX_CMD.format(self.SGTX_BLOCKS, self.SGTX_BENG_FRAMES_PER_BLOCK)
		with open("stdout.log_tx.{0}".format(sgtx_pattern), "w") as stdout:
			with open("stderr.log_tx.{0}".format(sgtx_pattern), "w") as stderr:
				self._process = Popen(self.SGTX_CMD.format(self.SGTX_BLOCKS, self.SGTX_BENG_FRAMES_PER_BLOCK).split(), stdout=stdout, stderr=stderr, env=self.env)
