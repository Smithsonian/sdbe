	...
	for this_file in input_filenames:
		...
		with open(this_file,'r') as fh:
			while True:
				...
				if (this_frame.bcount < bcount_start):
					t0 = datetime.now()
					...
					if (this_frame.bcount < (bcount_start-1)):
						bcount_deficit = (bcount_start-1) - this_frame.bcount
					...
					T_overhead = T_overhead + (datetime.now() - t0).total_seconds()
					continue
				elif (this_frame.bcount >= bcount_end):
					# we got all the bcount
					logger.debug('bcount is %d (> %d), stop reading from %s' % (this_frame.bcount,bcount_end,this_file))
					break
					
				# find absolute channel positions
				t0 = datetime.now()
				chan_id = this_frame.chan_id 
				fid = this_frame.fid
				...
				start_snap = (this_frame.bcount-bcount_start)*SWARM_TRANSPOSE_SIZE
				stop_snap = start_snap+SWARM_TRANSPOSE_SIZE
				
				logger.debug('bcount = %d, chan_id = %d, fid = %d: [start_snap:stop_snap, start_chan:stop_chan] = [%d:%d, %d:%d]' % (this_frame.bcount,chan_id,fid,start_chan,stop_chan,start_snap,stop_snap))
...
def main():
	...
	return read_spectra_from_files('prep6_test1_local_swarmdbe',1,1)

if __name__ == '__main__':
	spectra_real,spectra_imag,timestamps_at_first_usable_packet = main()
